...
export function useAgentSelection() {
    ...
    const setSelectedAgentName = useCallback(
        (newAgentName: string) => {
            ...
            if (selectedAgentConfigSet) {
                ...
                if (!agentExists) {
                    window.elasticApm?.captureError(`Agent "${newAgentName}" not found in config set!`)
                    console.error(`[AGENT-SELECTION] ERROR: Agent "${newAgentName}" not found in config set!`)
                    console.log(
                        `[AGENT-SELECTION] Available agents: ${selectedAgentConfigSet.map((a) => a.name).join(", ")}`,
                    )
                    return // Don't set invalid agent names
                }
            } else {
                console.warn(
                    `[AGENT-SELECTION] Warning: selectedAgentConfigSet is null when setting agent to "${newAgentName}"`,
                )
            }
